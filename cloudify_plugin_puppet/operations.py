...
from cloudify_plugin_puppet.manager import *
...
def operation(ctx, **kwargs):
    ...
    if ops_tags:
        ...
        if not isinstance(op_tags, list):
            raise PuppetParamsError("Operation tags must be a list, not {0}".
                format(op_tags))
        if op_tags:
            ...
            ctx.logger.info("No tags specific to operation '{0}'".format(op))
